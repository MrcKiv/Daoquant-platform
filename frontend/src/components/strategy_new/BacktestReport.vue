...
<template>
  <div class="report-toolbar" v-if="hasBacktestData">
    <button class="download-btn" :disabled="isGenerating || !reportReady" @click="handleGeneratePDF">
...
<!--          <thead>-->
<!--            <tr style="background-color: #f2f2f2;">-->
<!--              <th style="padding: 10px; border: 1px solid #ddd; text-align: left;width: 15%;">指标</th>-->
<!--              <th style="padding: 10px; border: 1px solid #ddd; text-align: left;">值</th>-->
<!--            </tr>-->
<!--          </thead>-->
<!--          <tbody>-->
...
<div class="metrics-table" style="margin: 15px 0;">
  <table style="width: 100%; border-collapse: collapse; border: 1px solid #ddd;">
    <tbody>
      <tr>
        <td style="padding: 10px; border: 1px solid #ddd; background-color: #f2f2f2; text-align: center; font-weight: bold; width: 10%;">指标</td>
        <td style="padding: 8px; border: 1px solid #ddd; text-align: center;font-weight: bold; width: 18%;">最大回撤</td>
        <td style="padding: 8px; border: 1px solid #ddd; text-align: center;font-weight: bold; width: 18%;">年化收益</td>
        <td style="padding: 8px; border: 1px solid #ddd; text-align: center;font-weight: bold; width: 18%;">夏普率</td>
        <td style="padding: 8px; border: 1px solid #ddd; text-align: center;font-weight: bold; width: 18%;">胜率</td>
        <td style="padding: 8px; border: 1px solid #ddd; text-align: center;font-weight: bold; width: 18%;">索提诺比率</td>
      </tr>
      <tr>
        <td style="padding: 10px; border: 1px solid #ddd; background-color: #f2f2f2; text-align: center; font-weight: bold; width: 10%;">值</td>
        <td style="padding: 8px; border: 1px solid #ddd; text-align: center;font-weight: bold; width: 18%;">{{ formatPercentage(metrics?.strategy?.maxDrawdown || 0) }}%</td>
        <td style="padding: 8px; border: 1px solid #ddd; text-align: center;font-weight: bold; width: 18%;">{{ formatPercentage(metrics?.strategy?.annualizedReturn || 0) }}%</td>
        <td style="padding: 8px; border: 1px solid #ddd; text-align: center;font-weight: bold; width: 18%;">{{ (metrics?.strategy?.sharpeRatio || 0).toFixed(2) }}</td>
        <td style="padding: 8px; border: 1px solid #ddd; text-align: center;font-weight: bold; width: 18%;">{{ formatPercentage(metrics?.strategy?.winRate || 0) }}%</td>
        <td style="padding: 8px; border: 1px solid #ddd; text-align: center;font-weight: bold; width: 18%;">{{ (metrics?.strategy?.sortinoRatio || 0).toFixed(2) }}</td>
      </tr>
    </tbody>
  </table>
</div>
      ...
      <h4 style="font-size: 16px; font-weight: bold; margin: 15px 0 10px;">骞村害鍥炴姤</h4>
...
<!--      <div id="annual-returns-chart" style="width: 100%; height: 400px; background: white; margin: 15px 0; border: 1px solid #ddd;"></div>-->
      ...
      <h4 style="font-size: 16px; font-weight: bold; margin: 15px 0 10px;">鎸佽偂鏁伴噺鏃跺簭</h4>
...
<!--      <div id="holdings-timeline-chart" style="width: 100%; height: 400px; background: white; margin: 15px 0; border: 1px solid #ddd;"></div>-->
      ...
      <h4 style="font-size: 16px; font-weight: bold; margin: 15px 0 10px;">浜ゆ槗绫诲瀷鍒嗘瀽锛堜拱鍏ワ級</h4>
      ...
      <h4 style="font-size: 16px; font-weight: bold; margin: 15px 0 10px;">浜ゆ槗绫诲瀷鍒嗘瀽锛堝崠鍑猴級</h4>
...
<!--      <div id="transaction-type-analysis-sell-chart" style="width: 100%; height: 400px; background: white; margin: 15px 0; border: 1px solid #ddd;"></div>-->
      ...
      <div v-else class="empty-backtest-tip" style="padding: 50px; text-align: center; background-color: #f9f9f9; border: 1px solid #ddd; border-radius: 4px; margin-top: 20px;">
        <div style="color: #909399; font-size: 14px;">
          <p style="margin-bottom: 15px; font-size: 40px;">📊</p>
          <p style="font-size: 18px; font-weight: bold; color: #333;">当前策略尚未生成回测结果</p>
          <p style="font-size: 14px; margin-top: 10px; color: #666;">请先点击"运行回测"生成数据，或检查策略参数设置</p>
        </div>
      </div>
    </div>
...
const backtestResult = ref(null) // Initialize as null
...
const restoreLatestBacktest = () => {
  ...
  try {
    ...
    backtestResult.value = snapshot.backtestResult || null
    metrics.value = snapshot.backtestResult?.metrics || {}
    return !!(backtestResult.value && Array.isArray(backtestResult.value.dates) && backtestResult.value.dates.length > 0)
  } catch (error) {
    console.error('恢复最近回测快照失败:', error)
    return false
  }
}

// Check if valid backtest data exists
const hasBacktestData = computed(() => {
  return (
    backtestResult.value &&
    Array.isArray(backtestResult.value.dates) &&
    backtestResult.value.dates.length > 0
  )
})
// 从后端获取的真实交易数据
const realTrades = computed(() => {
  const trades = []
  if (backtestResult.value && backtestResult.value.trades) {
    // 遍历交易数据，按日期组织
...
const loadStrategyConfig = async () => {
  try {
    ...
    if (res.data.success) {
      ...
      if (res.data.backtest_result && Array.isArray(res.data.backtest_result.dates) && res.data.backtest_result.dates.length > 0) {
        backtestResult.value = res.data.backtest_result
      }
      
      if (backtestResult.value) {
        metrics.value = backtestResult.value.metrics || {}
      } else {
        metrics.value = {}
      }
      ...
      console.error('加载策略失败:', errorMessage)
      alert('加载策略失败: ' + errorMessage)
    }
  } catch (err) {
    console.error('请求失败:', err)
    const errorMessage = err.response?.data?.message || err.response?.data?.error || err.message || '请检查网络或稍后重试'
    alert('加载策略失败: ' + errorMessage)
  }
}
...
const handleGeneratePDF = async () => {
  if (!hasBacktestData.value || !reportReady.value) {
    return
  }
  await nextTick()
