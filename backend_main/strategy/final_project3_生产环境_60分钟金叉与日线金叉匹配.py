...
from strategy.策略.日线金叉与60分钟金叉匹配优化_加入MACD否决_v2 import MACDGoldenCrossMatcher
...
def buy_stock(stocklist, candidate_stock, max_stock_num, remained_money, current_day, fund_data, sid, uid):
    ...
    # 强制将 pct_chg 列转为 float
    if 'pct_chg' in invest_stock_data.columns:
        invest_stock_data.loc[:, 'pct_chg'] = pd.to_numeric(invest_stock_data['pct_chg'], errors='coerce').fillna(0.0)
    ...
    print("invest_balance:", invest_balance)
    # 根据市场强弱动态决定本轮实际投入预算。
    ...
    if num > 0:
        for i in range(len(candidate_stock)):
            ...
            if 100 * data['close'][0] >= max_cost or 100 * data['close'][0] >= remained_money or (
                ...
                sc.df_table_transaction = pd.concat([sc.df_table_transaction, pd.DataFrame([add_rows])], ignore_index=True)  # 忽略新行的索引，会自动分配一个索引以保证索引的连续性
...
def macd_cross_sell_strategy(candidate_sell_stock, current_day, sid, uid):
    ...
    for i in range(len(candidate_sell_stock)):
        ...
        current_data = current_data.reset_index(drop=True)
        down_current_price = current_data['low'][0]
        current_price = current_data['close'][0]

        # 计算涨跌幅
        price_change_ratio = (current_price - buy_price) / buy_price
        down_current_ratio = (down_current_price - buy_price) / buy_price
...
def sell_stock(stocklist, candidate_sell_stock, remained_money, current_day, sid, uid):
    ...
    for i in range(len(candidate_sell_stock)):
        data = stock_data[
            (stock_data['st_code'] == candidate_sell_stock['st_code'][i]) & (stock_data['trade_date'] == current_date_obj)].copy()
        data.reset_index(drop=True, inplace=True)
        
        # 强制将需要运算的列转为数值类型，防止字符串比对报错
        numeric_cols = ['close', 'open', 'high', 'low', 'pre_close', 'pct_chg', 'vol', 'amount']
        for col in numeric_cols:
            if col in data.columns:
                data.loc[:, col] = pd.to_numeric(data[col], errors='coerce').fillna(0.0)
        ...
        sc.df_table_transaction = pd.concat([sc.df_table_transaction, pd.DataFrame([add_rows])], ignore_index=True)
...
def order(All_stock_data, totalmoney, max_stock_num, start_date, end_date, Optionfactors, factors, Baseline,
    ...
    if 'st_code' not in All_stock_data.columns and 'stock_code' in All_stock_data.columns:
        All_stock_data = All_stock_data.rename(columns={'stock_code': 'st_code'})
    if 'st_code' not in All_stock_data.columns:
        raise KeyError(f"Missing stock code column in All_stock_data: {list(All_stock_data.columns)}")
    ...
    if last_backtest_date:
        ...
        if not sc.df_table_baseline.empty and last_backtest_date:
            print("sc.df_table_baseline", sc.df_table_baseline)
            print("last_backtest_date", last_backtest_date)
            print("last_backtest_date type:", type(last_backtest_date))
            print("trade_date column type:",
                  type(sc.df_table_baseline['trade_date'].iloc[0]) if not sc.df_table_baseline.empty else "Empty")

            # 调试信息
            if not sc.df_table_baseline.empty:
                print("trade_date values:", sc.df_table_baseline['trade_date'].tolist())
                print("last_backtest_date stripped:", repr(last_backtest_date.strip()))

                # 使用更健壮的比较方式
                last_backtest_date_clean = str(last_backtest_date).strip()
                sc.df_table_baseline['trade_date'] = sc.df_table_baseline['trade_date'].astype(str).str.strip()

                last_baseline = sc.df_table_baseline[sc.df_table_baseline['trade_date'] == last_backtest_date_clean]
                print("匹配结果:", last_baseline)
            else:
                last_baseline = pd.DataFrame()
                print("sc.df_table_baseline is empty")
            if not last_baseline.empty:
                print(3333)
                print("last_baseline", last_baseline)
                testmoney = last_baseline.iloc[0]['assets']  # 使用历史资产作为新的起始资金
    ...
    # ====== 根源性修复：在数据源头统一将所有数值列强制转为 float ======
    # 数据库读取后这些列可能是字符串，后续所有切片操作均会继承正确的数值类型
    numeric_cols_to_convert = ['open', 'high', 'low', 'close', 'pre_close', 'pct_chg', 'vol',
                                'cci', 'pre_cci', 'macd_macd', 'pre_macd_macd', 'MACD']
    ...
    while True:
        ...
        if not basedata_today.empty and len(basedata_today.values) > 0:
            ...
            if testmoney <  10 * today_open_price:
                ...
                # print('上证指数开盘价：', today_open_price)
                stocknum = int(testmoney / (10 * today_open_price))  # 买入的大盘股票数量
                # print('买入的大盘指标数量：', stocknum)
                testmoney = testmoney - stocknum * 10 * today_open_price
                # print('买完大盘之后剩下多少钱：', testmoney)
                reference_market_capitalization = stocknum * 10 * today_open_price
            # print('今天的大盘开盘价是多少', reference_market_capitalization)
            print('testmoney', testmoney)
            print('reference_market_capitalization', reference_market_capitalization)
        ...
        # try:
        #     # 获取当前日期的大盘开盘价
        #     basedata_current = Basedata[Basedata['trade_date'] == current_day]
        #     if not basedata_current.empty:
        #         current_open_price = basedata_current['open'].iloc[0]
        #         # 基于初始资金计算可以购买的大盘份额作为参考
        #         if current_open_price > 0:
        #             # 使用初始资金totalmoney计算参考份额，而不是当前资金testmoney
        #             reference_shares = int(totalmoney / (100 * current_open_price))
        #             reference_market_capitalization = reference_shares * 100 * current_open_price
        #         else:
        #             reference_market_capitalization = 0
        #     else:
        #         reference_market_capitalization = 0
        #         print(f"无法获取 {current_day} 的大盘数据")
        # except Exception as e:
        #     reference_market_capitalization = 0
        #     print(f"计算大盘参考市值时出错: {e}")
        #
        # # 计算总资产和盈亏情况（基于参考的大盘表现）
        # assets = testmoney + reference_market_capitalization
        # profit_and_loss = assets - totalmoney
        # profit_and_loss_ratio = profit_and_loss / totalmoney if totalmoney != 0 else 0
        #
        # # 记录参考数据
        # add_rows = {
        #     'trade_date': current_day,
                ...
                if CHAIDAN == 1:
                    ...
                    for _, row in executed_result.iterrows():
                        ...
                        split_order_record = {
                            ...
                            'number_of_transactions': row['executed_volume'],  # 转换为股数
                    ...
                    if new_transactions:
                        ...
                        new_transactions_df['number_of_transactions'] = (
                            pd.to_numeric(new_transactions_df['number_of_transactions'], errors='coerce')
                            .fillna(0)
                            .astype(int)
                        )
        ...
        if len(stocklist) < max_stock_num:
            ...
            # 强制将 pct_chg 列转为 float 以避免后续发生 DataFrame Pandas 列内的字符串比较报错
            if 'pct_chg' in selected_stock.columns:
                selected_stock['pct_chg'] = pd.to_numeric(selected_stock['pct_chg'], errors='coerce').fillna(0.0)
            ...
            if len(selected_stock) > 0:
                ...
                candidate_buy_stock = candidate_buy_stock.drop_duplicates(subset=['st_code']).reset_index(drop=True)
        ...
        if len(stocklist) > 0:
            for i in range(len(stocklist)):
                ...
                # 强制将交易记录中的关键字段转为数值类型
                cost_price = float(pd.to_numeric(hold_stock_info['trade_price'][0], errors='coerce') or 0.0)
                num_transactions = float(pd.to_numeric(hold_stock_info['number_of_transactions'][0], errors='coerce') or 0.0)
                current_price = float(current_close['close'][0])

                # 计算盈亏
                profit_and_loss = (current_price - cost_price) * num_transactions
                # 计算盈亏率
                profit_and_loss_ratio = (current_price - cost_price) / cost_price if cost_price != 0 else 0
                latest_value = current_price * num_transactions
                add_rows = (
                    {'trade_date': current_day, 'st_code': stocklist['st_code'][i],
                     "number_of_securities": num_transactions,
                     "saleable_quantity": num_transactions,
                     "cost_price": cost_price,
                     "profit_and_loss": profit_and_loss, 'profit_and_loss_ratio': profit_and_loss_ratio,
                     'latest_value': latest_value, 'current_price': current_price,
                     'strategy_id': sid, 'user_id': uid})

                sc.df_table_shareholding = pd.concat([sc.df_table_shareholding, pd.DataFrame([add_rows])], ignore_index=True)
        # 即使没有持仓也要确保当日有记录
        else:
            # 可以添加空记录或零值记录，确保数据完整性
            pass

        # ====== 修复：每日统计表使用 remained_money（逐笔跟踪的真实现金余额） ======
        # 之前的计算方式 `balance = totalmoney + turnover_sell - turnover_buy` 
        # 会因交易记录中的类型问题/重复记录导致 balance 严重偏离真实值
        balance = float(remained_money)
        available = balance
        ...
        if not sc.df_table_shareholding.empty:
            ...
            if not current_holdings.empty:
                # 确保 latest_value 为数值类型再求和
                reference_market_capitalization = pd.to_numeric(
                    current_holdings['latest_value'], errors='coerce').fillna(0.0).sum()
        ...
        sc.df_table_statistic = pd.concat([sc.df_table_statistic, pd.DataFrame([add_rows])], ignore_index=True)
        print(f'每日统计数据: {current_day} - 余额: {balance:.2f}, 持仓市值: {reference_market_capitalization:.2f}, 总资产: {assets:.2f}')

        count_day = count_day + 1
        print(666666666666666666)
        print("count_day",count_day)

        print("trade_day",trade_day)
        print("trade_day1",trade_day1)
        print("type(trade_day1)",type(trade_day1))
        print("int(trade_day1)",int(trade_day1))
        print("int(trade_day) + count_day",int(trade_day) + count_day)
    ...
    if len(stocklist) > 0:
        ...
        for i in range(len(stocklist)):
            ...
            data = sc.df_table_daily_qfq[(sc.df_table_daily_qfq['st_code'] == stocklist['st_code'][i]) & (
                    sc.df_table_daily_qfq['trade_date'] == current_date_obj)].copy()
            data = data.reset_index(drop=True)
            
            # 强制将 close 转为数值类型
            if 'close' in data.columns:
                data['close'] = pd.to_numeric(data['close'], errors='coerce').fillna(0.0)
            ...
            try:
                ...
                data1 = sc.df_table_transaction[sc.df_table_transaction['st_code'] == data['st_code'][0]].sort_values(
                    by='trade_date', ascending=False)
                data1 = data1.reset_index(drop=True)
                # 强制将 number_of_transactions 转为数值类型
                if 'number_of_transactions' in data1.columns:
                    data1['number_of_transactions'] = pd.to_numeric(data1['number_of_transactions'], errors='coerce').fillna(0.0)
            ...
            if data1 is not None and not data1.empty and 'number_of_transactions' in data1.columns:
                tmp = tmp + float(data['close'][0]) * float(data1['number_of_transactions'][0])
    ...
    # 三张表写入数据库
    # 由于环境 Pandas 2.2.3 移除了对 SQLAlchemy 1.4 部分核心特性的支持导致 API 断层，
    # 我们彻底摒弃 to_sql，并改用您项目内现成的基于原生 PyMySQL 的 sc.save_df 来执行批量入库。
    sc.save_df(sc.df_table_transaction, "new_war_历史成交信息表")
    sc.save_df(sc.df_table_statistic, "new_war_每日统计表")
    sc.save_df(sc.df_table_shareholding, "new_war_当前持股信息表")
    sc.save_df(sc.df_table_baseline, "大盘盈亏表")
...
def daily_60m_cross_main(fund_data, InvestmentRatio, max_hold, start_date, end_date, Optionfactors, sell_policy_list, sid, uid):
    ...
    # 如果已有完整回测数据，直接返回结果
    if (not force_full_backtest) and last_backtest_date == end_date:
        print(f"策略 {sid} 已有完整回测数据，无需重复回测")
        # 从数据库获取最终结果
        sql = """
        SELECT assets FROM new_war_每日统计表 
    ...
    try:
        # 获取所有交易日列表
        trade_dates = sc.df_table_daily_qfq['trade_date'].unique()
        ...
        for trade_date in trade_dates:
            # 查询该交易日的ST股票
            st_stocks_on_date = sc.df_table_st_stock[sc.df_table_st_stock['trade_date'] == trade_date][
                'st_code'].tolist()

            # 找到All_stock_data中该交易日的ST股票数据
            st_in_all_stock = sc.df_table_daily_qfq[
                (sc.df_table_daily_qfq['trade_date'] == trade_date) &
                (sc.df_table_daily_qfq['st_code'].isin(st_stocks_on_date))
                ]

            # 记录需要剔除的索引
            if not st_in_all_stock.empty:
                stocks_to_drop.extend(st_in_all_stock.index.tolist())

        # 从All_stock_data中剔除ST股票
        if stocks_to_drop:
            sc.df_table_daily_qfq = sc.df_table_daily_qfq.drop(stocks_to_drop).reset_index(drop=True)
            print(f"已剔除 {len(stocks_to_drop)} 条ST股票数据")
    ...
    stock2 = df_daily_with_macd

    stock3 = stock2.groupby('st_code').apply(ml.calc, factor=factors)
    stock3 = stock3.reset_index()
    if 'level_1' in stock3.columns:
        stock3 = stock3.drop(columns=['level_1'])
    ...
    totalmoney = order(All_stock_data, fund_data * (InvestmentRatio / 100), max_hold, actual_start_date,
                       ...
                       last_backtest_date if (
                               (not force_full_backtest) and last_backtest_date and last_backtest_date < end_date and is_start_date_consistent) else None)
    end_time = time.time()
