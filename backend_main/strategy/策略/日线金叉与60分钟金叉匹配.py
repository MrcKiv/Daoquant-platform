...
class MACDGoldenCrossMatcher:
                    ...
                    (df_daily['macd_macd'] > df_daily['pre_macd_macd']) & (
                        df_daily['pre_macd_macd'] < df_daily['pre_pre_macd_macd']) & (
                                df_daily['pre_pre_macd_macd'] > 0) & (df_daily['macd_dif'] > 0)) | (
                                        (df_daily['macd_macd'] > df_daily['pre_macd_macd']) & (
                                            df_daily['pre_macd_macd'] < df_daily['pre_pre_macd_macd']) & (
                                                    df_daily['macd_macd'] < 0) & (df_daily['macd_dif'] < 0))
    ...
    def add_macd_score_to_daily_data(self, df_60m, df_daily):
        ...
        # 2. 找到日线金叉
        df_golden_cross_daily = self.find_golden_cross_daily_vectorized(df_daily)
        # 3. 获取完整匹配结果（含 match_score）
        df_match_results = self.match_golden_cross_complete(
            df_golden_cross_60m, df_golden_cross_daily, df_daily
        )

        # 只保留需要的字段
        matched_days = df_match_results[['stock_code', 'golden_cross_date_60m', 'is_matched', 'match_score']].copy()
        matched_days.rename(columns={
            'stock_code': 'st_code',
            'golden_cross_date_60m': 'trade_date'
        }, inplace=True)

        # ⚠️ 确保股票代码和日期格式统一
        matched_days['st_code'] = matched_days['st_code'].astype(str)
        df_daily['st_code'] = df_daily['st_code'].astype(str)

        matched_days['trade_date'] = pd.to_datetime(matched_days['trade_date']).dt.date
        df_daily['trade_date'] = pd.to_datetime(df_daily['trade_date']).dt.date

        # 先初始化为 -1
        df_daily['MACD'] = -1

        # merge 匹配，取 match_score
        ...
        print(
            f"日线数据与匹配结果合并后记录数: {len(df_daily)}，其中匹配上记录数: {df_daily['match_score'].notna().sum()}"
        )

        # 如果匹配到则取 match_score，否则保持 -1
