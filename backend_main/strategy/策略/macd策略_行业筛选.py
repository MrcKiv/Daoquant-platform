...
def calculate_macd_composite_score_vectorized(df, macd_dif_max_indexed, macd_dif_min_indexed, macd_max_indexed,
    ...
    # 新增筛选逻辑：phase_liuru或phase_liuchu至少有一个为1
    phase_condition = (df['phase_liuru'] == 1) | (df['phase_liuchu'] == 1)
    ...
    # 组合条件计算最终得分
    # 满足phase条件、日线金叉且满足周线动量条件
    full_condition = phase_condition & daily_cross & weekly_momentum
    scores.loc[full_condition] = macd_dif_temp.loc[full_condition]

    # 满足phase条件、日线金叉但不满足周线动量条件
    partial_condition = phase_condition & daily_cross & ~weekly_momentum
    scores.loc[partial_condition] = MAX_SCORE_DECREASE + macd_dif_temp.loc[partial_condition]

    # 不满足phase条件或者不满足日线金叉条件
    no_condition = ~phase_condition | ~daily_cross
...
def calculate_macd_composite_score(row, max_macd_dif, min_macd_dif, max_macd, min_macd, volatility_factor=1.0):
    ...
    # 新增筛选逻辑：phase_liuru或phase_liuchu至少有一个为1
    if row['phase_liuru'] != 1 and row['phase_liuchu'] != 1:
        # 如果不满足phase条件，直接返回负分
        return MIN_SCORE_DECREASE
