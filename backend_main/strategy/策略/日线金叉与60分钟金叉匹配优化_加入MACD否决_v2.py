...
class MACDGoldenCrossMatcher:
    ...
    def apply_macd_veto(self, df_daily, lookback=2):
        """
        MACD否决（只做一层过滤，不改变原筛选/打分逻辑）：
        - 仅在红柱(macd_macd>0)时考虑"高位动能回落"风险；
        - 当日红柱 < 昨日红柱（动能走弱）触发否决；
        - 可选：若提供 pre_pre_macd_macd，则要求连续回落（昨日 < 前日）更稳健；
        - 可选：若提供 macd_dif/last_dif，则 DIF 拐头向下 + 红柱回落 也触发否决。

        返回：
            在df中新增 bool 列 macd_veto（True=否决）
        """
        ...
        # 强制将相关 MACD 列转换为数值型，防止后续字符串比较报错
        numeric_cols = ['macd_macd', 'pre_macd_macd', 'pre_pre_macd_macd', 'macd_dif', 'last_dif']
        ...
        if not required_cols.issubset(df.columns):
            self.log("警告：日线数据缺少必要的MACD列，跳过MACD否决检查")
            return df

        # 基本否决条件：红柱且当日红柱小于昨日红柱
        hist_positive = df['macd_macd'] > 0
        hist_falling_today = df['macd_macd'] < df['pre_macd_macd']
        veto = hist_positive & hist_falling_today

        # 增强否决条件：要求连续回落（lookback >= 2）
        if lookback >= 2 and 'pre_pre_macd_macd' in df.columns:
            hist_falling_yesterday = df['pre_macd_macd'] < df['pre_pre_macd_macd']
            veto = veto & hist_falling_yesterday

        # DIF拐头向下 + 红柱回落的否决条件
        if 'macd_dif' in df.columns and 'last_dif' in df.columns:
            dif_turn_down = df['macd_dif'] < df['last_dif']
            veto_from_dif = hist_positive & hist_falling_today & dif_turn_down
            veto = veto | veto_from_dif

        df['macd_veto'] = veto.fillna(False)

        # 输出否决统计信息
        veto_count = df['macd_veto'].sum()
        self.log(f"MACD否决检查完成，共标记 {veto_count} 条记录为否决状态")
    ...
    def load_data(self, start_date=None, end_date=None):
        ...
        try:
            ...
            df_60m = sc.safe_read_sql(sql_60m)
            self.log(f"60分钟数据加载完成，记录数: {len(df_60m)}")
            print("60分钟数据:", len(df_60m))
            # 读取日线数据（增加close列用于计算股价上涨）
            self.log("读取日线数据...")
            sql_daily = """
            SELECT st_code, trade_date, close, macd_macd, pre_macd_macd, pre_pre_macd_macd,macd_dif,last_dif
            FROM stock_1d_all
            ...
            df_daily = sc.safe_read_sql(sql_daily)
    ...
    def find_golden_cross_60m_vectorized(self, df_60m, df_daily=None):
        ...
        # 使用更严格的向量化操作识别金叉，增加防止追高条件
        golden_cross_mask = (
                (df_60m['pre_macd'] < 0) & (df_60m['macd'] > 0) &  # 基本金叉条件
                ((df_60m['dif'] - df_60m['pre_dif']) / (df_60m['pre_dif'] - df_60m['pre_pre_dif']) > 1.2) &  # DIF斜率条件
                # (df_60m['volume'] > 1.2 * df_60m['vol_ma5']) &  # 成交量放大条件
                ((df_60m['close'] > df_60m['ma5']) | (df_60m['close'] > df_60m['ma10'])) &  # 均线位置条件
                (df_60m['close'] / df_60m['close'].shift(5) < 1.05)  # 防止追高（5日内涨幅不超过8%）
        )
        ...
        if df_daily is not None and not df_daily.empty:
            ...
            # 确保日线数据包含所需列
            required_columns = ['macd_dif', 'last_dif', 'st_code', 'trade_date', 'macd_macd', 'pre_macd_macd']
            if set(required_columns).issubset(df_daily.columns):
                # 转换日线数据日期格式以匹配60分钟数据
                df_daily_copy = df_daily.copy()
                df_daily_copy['trade_date'] = pd.to_datetime(df_daily_copy['trade_date']).dt.date
                df_daily_copy['condition_met'] = (
                        (df_daily_copy['macd_dif'] > df_daily_copy['last_dif']) &
                ...
                self.log(f"根据每个金叉日期的日线条件过滤掉 {filtered_count} 条记录")
            else:
                missing_cols = [col for col in required_columns if col not in df_daily.columns]
    ...
    def find_golden_cross_daily_vectorized(self, df_daily):
        ...
        # 强制将相关 MACD 列转换为数值型，防止字符串与数字比较时报错
        numeric_cols = ['macd_macd', 'pre_macd_macd', 'pre_pre_macd_macd', 'macd_dif', 'week_macd_dif', 'lastweek_macd_dif', 'lastlastweek_macd_dif']
        for col in numeric_cols:
        ...
        #         row['lastlastweek_macd_dif'] - row['lastweek_macd_dif']))
        # 使用向量化操作识别金叉
        golden_cross_mask = (((df_daily['pre_macd_macd'] < 0) & (df_daily['macd_macd'] > 0)) | (
                    (df_daily['macd_macd'] > df_daily['pre_macd_macd']) & (
                        df_daily['pre_macd_macd'] < df_daily['pre_pre_macd_macd']) & (
                                df_daily['pre_pre_macd_macd'] > 0) & (df_daily['macd_dif'] > 0)) | (
                                        (df_daily['macd_macd'] > df_daily['pre_macd_macd']) & (
                                            df_daily['pre_macd_macd'] < df_daily['pre_pre_macd_macd']) & (
                                                    df_daily['macd_macd'] < 0)
                                        & (df_daily['macd_dif'] < 0)) & ((df_daily['week_macd_dif'] - df_daily['lastweek_macd_dif'] )
                             > 1.25 * (df_daily['lastweek_macd_dif'] - df_daily['lastlastweek_macd_dif'])))
    ...
    def load_calendar_data(self):
        ...
        try:
            ...
            df_calendar = sc.safe_read_sql(sql)
    ...
    def add_macd_score_to_daily_data(self, df_60m, df_daily):
        ...
        df_daily_crosses = df_golden_cross_daily[['stock_code', 'trade_date']].copy()

        # 4. 确保股票代码和日期格式统一
        df_60m_crosses['stock_code'] = df_60m_crosses['stock_code'].astype(str)
        df_daily_crosses['stock_code'] = df_daily_crosses['stock_code'].astype(str)
        ...
        # 11.5 增加一层MACD否决过滤（不改变原逻辑，仅剔除“红柱高位回落”的候选）
        df_daily = self.apply_macd_veto(df_daily, lookback=2)
        if 'macd_veto' in df_daily.columns:
            veto_mask = (df_daily['MACD'] > 0) & (df_daily['macd_veto'] == True)
            veto_count = int(veto_mask.sum())
            if veto_count > 0:
                print(f"MACD否决过滤触发数: {veto_count}，这些候选将被置为-1剔除")
            df_daily.loc[veto_mask, 'MACD'] = -1
