...
def compute_scores(df, config=StrategyConfig):
    ...
    if 'vol' in df.columns:
        ...
        print(f"vol列统计: 最小值={vol_series.min()}, 最大值={vol_series.max()}, 均值={vol_series.mean():.2f}")
        
        # 检查数据有效性
        null_count = vol_series.isnull().sum()
        inf_count = np.isinf(vol_series).sum()
        neg_count = (vol_series < 0).sum()
        
        print(f"vol列质量: NaN={null_count}, 无穷大={inf_count}, 负值={neg_count}")
        ...
        if vol_series.isnull().all():
            ...
            vol_series_clean = vol_series_clean.clip(lower=0)  # 成交量不能为负
            
            print(f"清理后vol列统计: 最小值={vol_series_clean.min()}, 最大值={vol_series_clean.max()}, 均值={vol_series_clean.mean():.2f}")
            
            vol_series_clean.name = 'vol_series'
