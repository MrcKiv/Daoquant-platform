...
def queryClosePrice(request):
    ...
    today_close = sc.safe_read_sql(sql)
...
def getSimilar(request):
    ...
    results = find_similar_stocks_adaptive(target_stock, all_stocks, datetime.strptime(startDate, '%Y%m%d').date(),
                                           datetime.strptime(endDate, '%Y%m%d').date(),
                                           window_size_range=(0.9, 1.1),use_ddtw= True)
    # 按相似度从高到低排序后再取前10条
    ...
    for _, row in top_10.iterrows():
        ...
        if hasattr(row["起始日期"], 'strftime'):
            start_date_str = row["起始日期"].strftime("%Y-%m-%d")
        else:
            start_date_str = datetime.strptime(row["起始日期"], "%Y%m%d").strftime("%Y-%m-%d")
        if hasattr(row["终止日期"], 'strftime'):
            end_date_str = row["终止日期"].strftime("%Y-%m-%d")
        else:
            end_date_str = datetime.strptime(row["终止日期"], "%Y%m%d").strftime("%Y-%m-%d")

        processed_results.append({
            "code": row["股票代码"],
            "startDate": row["起始日期"].strftime("%Y-%m-%d"),
            "endDate":  row["终止日期"].strftime("%Y-%m-%d"),
