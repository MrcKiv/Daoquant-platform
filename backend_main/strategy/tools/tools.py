...
from backend.env import get_sqlalchemy_database_url
...
def get_db_engine():
    ...
    if _db_engine is None:
        _db_engine = create_engine(
            get_sqlalchemy_database_url(charset="utf8"),
            poolclass=NullPool,
            pool_recycle=3600,
...
def find_next_open_date(engine, initial_date_str):
    ...
    def query_specific_date(date_str):
        ...
        FROM 股票日历_new
        WHERE cal_date = '{date_str}'
        """
        return sc.safe_read_sql(sql)
    ...
    while result_df.empty:
        loop_count += 1
        if loop_count > 365:
            print(f"❌ find_next_open_date 超过最大查找次数 (365)，停止查找。Initial date: {initial_date_str}")
            return initial_date_str  # 返回原始日期，避免死循环
    ...
    if not result_df.empty:
        ...
        while is_open == '0':
            loop_count_open += 1
            if loop_count_open > 365:
                 print(f"❌ find_next_open_date (is_open) 超过最大查找次数 (365)。")
                 break
...
def buy_stock(stocklist, candidate_stock, max_stock_num, remained_money, current_day, fund_data,sid, uid):
    ...
    if total_changes > 0:
        balance1 = positive_changes / total_changes
    else:
        balance1 = 0.0 # 默认值，避免除零错误
...
class SaleCondition:
    ...
    def up(self):
        ...
        for i in range(len(stocklist)):
            ...
            if old_close.empty:
                continue
            condition = ((current_close['high'].iloc[0] - old_close['trade_price'].iloc[0]) / old_close['trade_price'].iloc[
    ...
    def down(self):
        ...
        for i in range(len(stocklist)):
            ...
            condition = (current_close['low'].iloc[0] - old_close['trade_price'].iloc[0]) / old_close['trade_price'].iloc[
...
def sell_stock(stocklist, candidate_sell_stock, remained_money, current_day, sid, uid):
    for i in range(len(candidate_sell_stock)):
        ...
        # Safety check for old_close
        if old_close.empty:
            print(f"Warning: No transaction history found for selling stock {candidate_sell_stock['st_code'][i]}")
            continue
...
def order(All_stock_data, totalmoney, max_stock_num, start_date, end_date, Optionfactors, factors, Baseline,
    ...
    # 强制将 uid/sid 转为 string，防止 1292 Error
    sid = str(sid)
    uid = str(uid)
    ...
    # 修复 Basedata 为空导致的 crash
    if Basedata.empty:
        print("警告：Basedata (大盘数据) 为空，初始化默认值以避免崩溃")
    ...
    while True:
        ...
        # 大盘指标
        # 安全获取大盘开盘价
        if not Basedata.empty and 'open' in Basedata.columns and len(Basedata['open']) > 0:
            first_open = Basedata['open'].iloc[0]
        else:
            first_open = 1.0 # 默认值防止除零或比较错误

        if testmoney < 100 * first_open:

            add_rows = {
                'trade_date': current_day,
                'reference_market_capitalization': 0,
                'assets': testmoney,
                'profit_and_loss': testmoney - totalmoney,
                'profit_and_loss_ratio': (testmoney - totalmoney) / totalmoney if totalmoney != 0 else 0,
                'strategy_id': sid, 'user_id': uid
            }
            
            # 尝试获取基准数据进行正确计算
            base_open_series = Basedata.loc[Basedata['trade_date'] == current_day, 'open']
            if not base_open_series.empty:
                 base_open = base_open_series.values[0]
                 add_rows['reference_market_capitalization'] = stocknum * 100 * base_open
                 add_rows['assets'] = testmoney + add_rows['reference_market_capitalization']
                 add_rows['profit_and_loss'] = add_rows['assets'] - totalmoney
                 add_rows['profit_and_loss_ratio'] = add_rows['profit_and_loss'] / totalmoney if totalmoney != 0 else 0
            ...
            base_open_series = Basedata.loc[Basedata['trade_date'] == current_day, 'open']
            if not base_open_series.empty:
                current_base_open = base_open_series.values[0]
            else:
                 # 如果当日无大盘数据，尝试获取第一条或设为1
                 current_base_open = first_open

            stocknum = int(testmoney / (100 * current_base_open)) if current_base_open > 0 else 0 # 买入的大盘股票数量
            # print('买入的大盘指标数量：', stocknum)
            testmoney = testmoney - stocknum * 100 * current_base_open
            # print('买完大盘之后剩下多少钱：', testmoney)
            reference_market_capitalization = stocknum * 100 * current_base_open
        ...
        if not sc.df_table_transaction.empty:
            ...
            turnover_buy = transaction_up_to_today[transaction_up_to_today['trade_type'] == '买入'][
                'turnover'].sum()
        print('current_day', current_day, 'turnover_sell', turnover_sell, 'turnover_buy', turnover_buy)
        # 计算账户余额
    ...
    # 三张表写入数据库
    # sc.df_table_transaction.to_sql("new_war_历史成交信息表", con=get_db_engine(), schema="jdgp", index=False,
    #                                index_label=False,
    #                                if_exists='append', chunksize=100)
    sc.save_df(sc.df_table_transaction, "new_war_历史成交信息表")

    # sc.df_table_statistic.to_sql("new_war_每日统计表", con=get_db_engine(), schema="jdgp", index=False, index_label=False,
    #                              if_exists='append', chunksize=100)
    sc.save_df(sc.df_table_statistic, "new_war_每日统计表")

    # sc.df_table_shareholding.to_sql("new_war_当前持股信息表", con=get_db_engine(), schema="jdgp", index=False,
    #                                 index_label=False,
    #                                 if_exists='append', chunksize=100)
    sc.save_df(sc.df_table_shareholding, "new_war_当前持股信息表")

    # sc.df_table_baseline.to_sql("大盘盈亏表", con=get_db_engine(), schema="jdgp", index=False, index_label=False,
    #                             if_exists='append', chunksize=100)
