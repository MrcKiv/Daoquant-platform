...
class ActiveWork():
    def __init__(self):
        ...
        if gl.gl_queue_DF_Data is None:
            csv_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'ActiveWork.csv')
            gl.gl_queue_DF_Data = pd.read_csv(csv_path, sep='\t')
    ...
    def edit_queue_the_one_status(self, key, path=None):
        if path is None:
            path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'ActiveWork.csv')
        ...
        field = ["key", "strategy_no", "stock_no", "stock_name","amount","price", "operate", "status"]
