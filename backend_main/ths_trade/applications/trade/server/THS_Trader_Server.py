...
class THSTraderServer:
    ...
    def __init__(self, exe_path=API_Config.cfg['exe_path']):  # r"C:\同花顺软件\同花顺\xiadan.exe"
        ...
        try:
            ...
            print(f"Warning: Failed to connect to trading client at {exe_path}. Error: {e}")
            self.app = None
            self.main_wnd = None
    # def __init__(self, config=None):
    #     """初始化交易服务器
    #     Args:
    #         config: 账户配置，如果为None则使用默认配置
    #     """
    #     if config is None:
    #         self.config = API_Config.cfg
    #     else:
    #         self.config = config
    #
    ...
    def buy(self, item):
        ...
        try:
            print("设置买入价格")
            price_control = self.main_wnd.window(control_id=0x409, class_name="Edit")
            price_control.set_text("")  # 先清空
            time.sleep(API_Config.cfg["sleepC"])
            price_control.type_keys(str(item['price']))  # 再输入
        except:
            time.sleep(API_Config.cfg["sleepC"])
            price_control = self.main_wnd.window(control_id=0x409, class_name="Edit")
            price_control.set_text("")
            time.sleep(API_Config.cfg["sleepC"])
            price_control.type_keys(str(item['price']))
        time.sleep(API_Config.cfg["sleepA"])
    ...
    def get_balance(self):
        """ 获取资金情况 """
        if self.app is None: return pd.DataFrame()
        self.app.top_window().set_focus()
        time.sleep(API_Config.cfg["sleepA"])
        try:
