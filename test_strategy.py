...
print("DEBUG: Importing mysql_connect", flush=True)
try:
    import backend_main.strategy.mysql_connect as sc
except Exception:
    import traceback
    traceback.print_exc()
...
from backend_main.strategy.tools.tools import order, check_existing_backtest_data, check_start_date_consistency, find_next_open_date
...
if __name__ == "__main__":
    ...
    # 本地测试配置
    # 数据库连接 (参考 macd 策略配置)
    print("DEBUG: Setting up DB", flush=True)
    ...
    try:
        print("DEBUG: Finding open date", flush=True)
    ...
    try:
        print("DEBUG: Clearing old data", flush=True)
        for sql in sql_list:
            sc.execute_sql(sql)
    except Exception as e:
    ...
    print("DEBUG: Calling main strategy function", flush=True)
